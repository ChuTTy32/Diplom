...
#   4. Генерирует WireGuard ключи (если не существуют)
#   5. Собирает образы и запускает сервисы
#   6. Ждёт healthcheck и показывает статус
#
...
# WireGuard (не критично для запуска, только для VPN)
if ! command -v wg &>/dev/null; then
    warn "wireguard-tools не найден — VPN не будет работать"
    warn "  sudo apt-get install wireguard-tools"
    WG_AVAILABLE=false
else
    ok "WireGuard $(wg --version 2>/dev/null || echo 'ok')"
    WG_AVAILABLE=true
fi
...
mkdir -p backend/app/api backend/app/core backend/app/schemas
ok "backend/app/* (структура пакетов)"
...
# ═══════════════════════════════════════════════════════════════════════
# ШАГ 4 — WIREGUARD КЛЮЧИ
# ═══════════════════════════════════════════════════════════════════════
step "WireGuard ключи"

if [ "$WG_AVAILABLE" = false ]; then
    warn "Пропуск — wireguard-tools не установлен"
elif [ -f "wireguard/config/wg1.conf" ]; then
    ok "Конфиги уже существуют — пропуск"
else
    warn "Генерируем ключи..."
    bash wireguard/scripts/gen_keys.sh
    ok "WireGuard ключи и конфиги созданы"
fi

# ═══════════════════════════════════════════════════════════════════════
# ШАГ 5 — ЗАПУСК (если не --no-start)
# ═══════════════════════════════════════════════════════════════════════
